...
ALTER TABLE `products`
  ADD COLUMN `cat_id` BIGINT UNSIGNED DEFAULT NULL;
ALTER TABLE `products`
  ADD COLUMN `br_id` BIGINT UNSIGNED DEFAULT NULL;
...
SELECT
  *,
  (SELECT cat
   FROM cats
   WHERE cats.id = products.cat_id) AS cats,
  (SELECT brand
   FROM brands
   WHERE brands.id = products.br_id) AS brands
FROM products;
